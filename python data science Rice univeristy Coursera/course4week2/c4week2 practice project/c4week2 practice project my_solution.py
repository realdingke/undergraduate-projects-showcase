...
def get_county_attributes(svg_file_name):
    ...
    path_stringpairs=[(path.getAttribute('id'),path.getAttribute('d')) for path in doc.getElementsByTagName('path')]
    return path_stringpairs
...
def get_boundary_coordinates(boundary_data):
    ...
    Ignores 'M', 'L, 'z'
    """
    list1=boundary_data.split()
    list2=[]
    for item in list1:
        if ('M' not in item) and ('L' not in item) and ('z' not in item):
            itemlist=item.split(',')
            list3=[]
            for num in itemlist:
                return_num=float(num)
                list3.append(return_num)
            tuple3=(list3[0],list3[1])
            list2.append(tuple3)    
...
def process_county_attributes(svg_file_name, csv_file_name):
    ...
    """
    attributes_list=get_county_attributes(svg_file_name)
    list0=[]
    for pair in attributes_list:
        list1=[]
        FIPScode=pair[0]
        tuple_center=compute_county_center(get_boundary_coordinates(pair[1]))
        list1.extend([FIPScode,tuple_center[0],tuple_center[1]])
        list0.append(list1)
