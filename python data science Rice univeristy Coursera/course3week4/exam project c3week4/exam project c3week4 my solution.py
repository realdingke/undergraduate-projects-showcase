...
def top_player_ids(info, statistics, formula, numplayers):
    ...
    list0=[]
    for row in statistics:
        tuple0=(row[info["playerid"]],formula(info,row))
        list0.append(tuple0)
    list0.sort(key=lambda pair: float(pair[1]), reverse=True)
    return_list=list0[0:numplayers]
    return return_list
...
def lookup_player_names(info, top_ids_and_stats):
    ...
    """
    table0=read_csv_as_list_dict(info["masterfile"],info["separator"],info["quote"])
    return_list=[]
    for pair in top_ids_and_stats:
        for row in table0:
            if pair[0]==row[info["playerid"]]:
                string0="{0:.3f} --- {1} {2}"
                string1=string0.format(pair[1],row[info["firstname"]],row[info["lastname"]])
                return_list.append(string1)
    return return_list
...
def compute_top_stats_year(info, formula, numplayers, year):
    ...
    """
    stats0=read_csv_as_list_dict(info["battingfile"],info["separator"],info["quote"])
    stats1=filter_by_year(stats0,year,info["yearid"])
    stats2=top_player_ids(info,stats1,formula,numplayers)
    return lookup_player_names(info,stats2)
...
def compute_top_stats_career(info, formula, numplayers):
    ...
    """
    stats0=read_csv_as_list_dict(info["battingfile"],info["separator"],info["quote"])
    nested_stats1=aggregate_by_player_id(stats0,info["playerid"],info["battingfields"])
    stats1=[]
    for key,value in nested_stats1.items():
        stats1.append(value)
    stats2=top_player_ids(info,stats1,formula,numplayers)
    return lookup_player_names(info,stats2)
