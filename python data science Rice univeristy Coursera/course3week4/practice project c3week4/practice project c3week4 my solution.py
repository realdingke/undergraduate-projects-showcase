...
def select_columns(my_table, col_indices):
    ...
    for row in my_table:
        return_row=[]
        for colidx in col_indices:
            return_row.append(my_table[my_table.index(row)][colidx])
...
def sort_by_column(my_table, col_idx):
    ...
    """
    my_table.sort(key=lambda row: float(row[col_idx]), reverse=True)       #add float to the output of lambda function to ensure sorting on fractions
...
def test_part2_code():
    ...
    print(read_csv_file("cancer_risk_trimmed.csv")==trimmed_risk_solution)
